...
public class CategoriaItem {
    ...
    public boolean cadastrar(CategoriaItem categoriaItem) {
        try (FileWriter fw = new FileWriter("categoriaitens.txt", true);
            ...
            bw.write(categoriaItem.getItem().getCodigo() + "," +
    ...
    public boolean editar(CategoriaItem categoriaItem) {
        ...
        try (FileWriter fw = new FileWriter("categoriaitens.txt");
            ...
            for (CategoriaItem ci : categoriaItens) {
                if (ci.getItem().getCodigo() == categoriaItem.getItem().getCodigo() &&
                        ci.getCategoria().getCodigo() == categoriaItem.getCategoria().getCodigo()) {
                    // Atualiza o CategoriaItem com os novos valores
                    bw.write(categoriaItem.getItem().getCodigo() + "," +
                            categoriaItem.getCategoria().getCodigo() + "," +
                            categoriaItem.getQuantidade());
                } else {
                    // Mantém o CategoriaItem existente no arquivo
                    bw.write(ci.getItem().getCodigo() + "," +
                            ci.getCategoria().getCodigo() + "," +
                            ci.getQuantidade());
                }
                bw.newLine();
            }
    ...
    public CategoriaItem consultar(CategoriaItem categoriaItem) {
        ...
        for (CategoriaItem ci : categoriaItens) {
            if (ci.getItem().getCodigo() == categoriaItem.getItem().getCodigo() &&
                    ci.getCategoria().getCodigo() == categoriaItem.getCategoria().getCodigo()) {
                return ci; // Retorna o CategoriaItem encontrado
            }
        }
